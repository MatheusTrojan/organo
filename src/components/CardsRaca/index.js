...
const CardsRaca = ({ raca, personagens, aoDeletar, mudarCor, aoFavoritar }) => {
    return (
        personagens.length > 0 && <section className='cards-raca' style={{ backgroundImage: 'url(/images/fundo.png)', backgroundColor: hexToRgba(raca.cor, 0.5) }}>
            <input onChange={evento => mudarCor(evento.target.value, raca.id)} value={raca.cor} type="color" className="input-cor" />
